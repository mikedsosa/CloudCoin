...
namespace CloudCoin
{
    class Blockchain
    {
        ...
        public void addPendingTransaction(Transaction transaction)
        {
            ...
            {
                throw new Exception("There must be sufficient money in wallet");
            }
            if (transaction.IsValid() == false)
            {
                throw new Exception("Cannot add an incalid transaction to a block");
            }
        ...
        public bool IsChainValid()
        {
            for (int i = 1; i < this.Chain.Count; i++)
            {
                ...
                //Check if current vlock hash is same as calculated hash
